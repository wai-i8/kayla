        ...
        <main className="main-content">
          {data.loading ? (
            ...
            <Suspense fallback={<div className="content-loading page-loading"><span /><p>載入頁面…</p></div>}>
              {view === 'today' && <TodayPage profile={data.profile} records={data.records} onAdd={openQuickAdd} onOpenRecords={openFilteredRecords} onOpenGuide={openGuide} onOpenPhotos={() => changeView('photos')} onOpenSettings={() => changeView('settings')} />}
              ...
              {view === 'photos' && <PhotosPage photos={album.photos} loading={album.loading} error={album.error} onAdd={album.addPhoto} onDelete={album.deletePhoto} />}
